...
open.main = {
	...
	callPageNote: function (scrollpos)
	{
		...
		if (scrollpos < 1300) 
		{
			...
			$('#message').html('<p>Get the word out and save the open internet. <a href="#">Tweet this website &raquo;</a></p>');	
		}			
		else if ((scrollpos > 1300) && (scrollpos < 2300)) 
		{
			$('#message').removeClass('green');
			$this.displayPageNote(0);
		}
		else if ((scrollpos > 2300) && (scrollpos < 3700)) 
		{
			$('#message').addClass('green');
			$this.displayPageNote(1);
		}		
		else if ((scrollpos > 3700) && (scrollpos < 4600)) 
		{
			$('#message').removeClass('green');
			$this.displayPageNote(2);
		}
		...
		{
			$('#message').removeClass('green');
			$this.displayPageNote(3);
		}
		else if ((scrollpos > 7900) && (scrollpos < 10000)) 
		{
			$('#message').addClass('green');
			$this.displayPageNote(4);
		}
		else if (scrollpos > 10000)
		{
			$this.showToolbar();
			$('#message').removeClass('green');
			$('#message').html('<p>Get the word out and save the open internet. <a href="http://twitter.com/home?status=A visual guide to the open internet: http://www.theopeninter.net" title="Tweet this site">Tweet this website &raquo;</a></p>');
		}
		else
		{
			$this.hideToolbar();
